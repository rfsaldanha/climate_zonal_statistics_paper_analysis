...
prec <- open_dataset(sources = "../brclim2/output_data/parquet/total_precipitation_sum.parquet") %>%
  ...
  summarise(value = sum(value, na.rm = TRUE)/54) %>%
  ungroup() %>%
  collect() %>%
  group_by(code_muni) %>%
...
koppen_map <- tm_shape(left_join(mun, kclim, by = "code_muni")) +
  tm_polygons(fill = "class", col_alpha = .1,
              fill.scale = tm_scale_categorical(values = "tableau.20"),
...
tmap_save(tm = koppen_map, filename = "koppen_map.png", dpi = 300)
